        ...
        extends AbstractBaseJnlpMojo
{
            ...
            throws MojoExecutionException, MojoFailureException
    {
        ...
        Set<ResolvedJarResource> allResolvedJarResources = new LinkedHashSet<>( resolvedCommonJarResources );
        ...
        for ( JnlpFile jnlpFile : jnlpFiles )
        {
            ...
            if ( signJnlp && StringUtils.isNotBlank( jnlpFile.getMainClass() ) && getSign() != null )
            {
                ...
                singedJnlpFile.setJarResources( new ArrayList<>( jnlpFile.getJarResources() ) );
            ...
            throws MojoExecutionException
    {
        ...
        Set<Artifact> transitiveVersionArtifacts = new LinkedHashSet<>();
        // artifacts resolved from repositories without version aatribute in final jnlp file
        Set<Artifact> transitiveUnversionedArtifacts = new LinkedHashSet<>();

        // sibling projects hit from a jar resources (need a special transitive resolution)
        Set<MavenProject> versionedSiblingProjects = new LinkedHashSet<>();
        Set<MavenProject> unversionedSiblingProjects = new LinkedHashSet<>();
                                   ...
                                   Set<Artifact> transitiveArtifacts, boolean outputVersionInJnlp )
    {
        for ( Artifact resolvedArtifact : transitiveArtifacts )
        {
            ...
            {
                getLog().debug(
                    "Add jarResource (transitive)(versioned): " + newJarResource + " " + outputVersionInJnlp );
